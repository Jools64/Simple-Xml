...
/*
    ...
    void xmlDocumentLoad(XmlDocument* d, char* filePath)
    ...
    XmlNode* xmlNodeGetNode(XmlNode* node, char* path, XmlDocument* d)
    XmlNode* xmlDocumentGetNode(XmlDocument* d, char* path)
...
void xmlAllocatorInit(XmlAllocatorBlock* allocator)
{
    ...
    a.memory = malloc(XML_ALLOCATOR_BLOCK_SIZE);
...
{
    XmlNode* node = xmlAllocatorAllocate(&d->allocator, sizeof(XmlNode));
...
{
    XmlAttribute* attribute = xmlAllocatorAllocate(&d->allocator, sizeof(XmlNode));
...
void xmlDocumentLoad(XmlDocument* d, char* filePath)
{
    ...
    d->buffer = xmlAllocatorAllocate(&d->allocator, XML_DOCUMENT_BUFFER_SIZE);
    d->documentPosition = 0;
...
void xmlStringBufferAppend(XmlStringBuffer* b, char* string, int count)
{
    ...
    else
    {
        ...
        if(b->next == 0)
        {
            b->next = xmlAllocatorAllocate(b->allocator, sizeof(XmlStringBuffer));
...
char* xmlStringBufferToString(XmlStringBuffer* b, XmlAllocatorBlock* a)
{
    ...
    char* string = xmlAllocatorAllocate(a, length+1);
...
char* xmlStringClone(char* s, XmlAllocatorBlock* a)
{
    char* string = xmlAllocatorAllocate(a, strlen(s) + 1);
    strcpy(string, s);
    return string;
}
